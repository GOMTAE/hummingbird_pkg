...
import hummingbird_hover_gt_env
...
register(
    id='HummingbirdHoverTaskEnvPPO-v4',
    entry_point='hummingbird_hover_task_gt_env_ppo_3rotors:HummingbirdHoverTaskEnv',
    max_episode_steps=max_episode_steps,
)

class HummingbirdHoverTaskEnv(hummingbird_hover_gt_env.HummingbirdHoverGtEnv):
    def __init__(self):
        """
        ...
        LoadYamlFileParamsTest(rospackage_name="hummingbird_pkg",
                               ...
                               yaml_file_name="hummingbird_gt_ppo_params_3rotors.yaml")  ### Need change
    ...
    def _set_action(self, action):
        ...
        self.step_counter += 1
        if self.step_counter > 100 + self.random_motor_failure * 50:
            motor_input[0] = 0
    ...
    def _is_done(self, observations):
        ...
        isout = self.is_out(current_position, current_distance, MAX_DISTANCE=5.1)
    ...
    def _compute_reward(self, observations, done):
        ...
        print('Current distance: {} | Current z-acceleration: {} | Reward: {} | Done: {}'.format(current_distance,
    ...
    def is_in_desired_boundary(self, current_position, bound=0.2):
        ...
        return numpy.sqrt((self.desired_point.x - x_current) ** 2 + (self.desired_point.y - y_current) ** 2 + (
                    self.desired_point.z - z_current) ** 2) < bound
    ...
    def is_out(self, current_position, current_distance, MAX_DISTANCE):
        ...
        if abs(current_position.x) >= 1 or abs(current_position.y) >= 1 or current_distance >= MAX_DISTANCE:
            isout = True
